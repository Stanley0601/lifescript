...
export default function EndingView({ char, endingId, relationship, onRestart, onHome }: {
  ...
  return (
    <motion.div className="min-h-full px-5 py-10" style={{ background: QQ_BG }}
      ...
      <div className="max-w-md mx-auto">
        ...
        <motion.div className="bg-white rounded-xl p-5 mb-3 shadow-sm"
          ...
          <div className="mt-4 rounded-xl px-4 py-3" style={{ background: `${QQ_BLUE}08` }}>
            <p className="text-[13px] mb-1" style={{ color: QQ_BLUE }}>命运落点</p>
        ...
        <motion.div className="bg-white rounded-xl p-5 mb-3 shadow-sm"
          ...
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs text-[#888]">你们的关系</p>
            <span className="text-[13px] px-2.5 py-1 rounded-full text-white" style={{ background: getRelationshipColor(relationship?.stage) }}>
          ...
          <div className="space-y-3">
            {dims.map(([label, value], i) => (
              <div key={label} className="flex items-center gap-2.5">
                <span className="text-[14px] text-[#888] w-10 text-right">{label}</span>
                ...
                <span className="text-[14px] font-medium text-[#333] w-6 text-right">{String(value)}</span>
              </div>
            ))}
          </div>
        </motion.div>
        ...
        <motion.div className="bg-[#111827] rounded-xl p-5 mb-3 text-white shadow-sm"
          ...
          <p className="text-[13px] text-white/60 mb-2">当前结局摘要</p>
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[18px] font-semibold">{ending.emoji} {ending.title}</p>
              <p className="text-[14px] text-white/70 mt-1 leading-relaxed">这不是唯一答案，但它是你这轮对话真正推出来的版本。</p>
            </div>
            <div className="text-right">
              <p className="text-[22px] font-bold">{Math.round(dims.reduce((sum, [, value]) => sum + value, 0) / dims.length)}</p>
              <p className="text-[13px] text-white/55">综合状态</p>
            </div>
          </div>
        </motion.div>
            ...
            <div className="space-y-2.5">
              {otherEndings.map((alt) => (
                <div key={alt.endingId} className="p-3 bg-[#f9f9f9] rounded-lg">
                  ...
                  <p className="text-[14px] text-[#666] leading-relaxed mb-2">{alt.description}</p>
                  {alt.outcome && (
                    <div className="rounded-lg border border-[#ececec] bg-white p-3 mb-2">
                      <div className="flex items-center justify-between gap-2 mb-1.5">
                        ...
                        <span className="text-[13px] px-2 py-1 rounded-full" style={{ background: `${QQ_BLUE}12`, color: QQ_BLUE }}>
                          平行版本
                        </span>
                      </div>
                      <p className="text-[14px] text-[#888] leading-relaxed mb-2">{alt.outcome.description}</p>
                      <div className="space-y-1.5">
                        {Object.entries(alt.outcome.stats).map(([label, value], index) => (
                          <div key={label} className="flex items-center gap-2">
                            <span className="text-[13px] text-[#999] w-8 text-right">{label}</span>
                            ...
                            <span className="text-[13px] text-[#555] w-6 text-right">{String(value)}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  )}
                  <p className="text-[14px] text-[#888] leading-relaxed">{alt.alternateText}</p>
                </div>
              ))}
            </div>
          </motion.div>
